...
import ua.footballdata.model.Areas;
...
public class AreaRestServiceImpl extends AbstractRestService<Area> {
	...
	public List<Area> findAllData() {
		...
		/*
		 * ResponseEntity<List<Area>> respEntity =
		 * restTemplate.exchange("http://api.football-data.org/v2/areas",
		 * HttpMethod.GET, this.getHttpEntitis(), new
		 * ParameterizedTypeReference<List<Area>>(){});
		 * logger.info("Get respEntity is null: " + (respEntity == null));
		 */

		ResponseEntity<Areas> respEntity = restTemplate.exchange("http://api.football-data.org/v2/areas",
				HttpMethod.GET, this.getHttpEntitis(), Areas.class);
		logger.info("Get respEntity is null: " + (respEntity == null));
		Areas areas = respEntity.getBody();
		apiRequestLimit.initByHeaders(respEntity.getHeaders());

		List<Area> areasList = areas.getAreas();
		return areasList;
	}
